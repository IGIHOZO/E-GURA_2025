...
const ShopAliExpress = () => {
  ...
  const getRelevantFilters = () => {
    ...
    if (selectedCategories.length === 0) {
      // Show all filters if no category selected
      return {
        showColors: true,
        showSizes: true,
        showBrands: true
    ...
    // Fashion, Sports, Beauty - show colors and sizes
    const showColors = selectedCategories.some(cat => 
      ['Fashion', 'Sports', 'Beauty', 'Toys'].includes(cat)
    );
    
    const showSizes = selectedCategories.some(cat => 
      ['Fashion', 'Sports'].includes(cat)
    );

    // All categories can have brands
    const showBrands = true;

    return { showColors, showSizes, showBrands };
  };
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (response.data.success && response.data.data.length > 0) {
        ...
        extractFilterOptions(mockProds);
      }
    } catch (error) {
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3 sm:space-x-6">
                ...
                <span className="text-xs sm:text-sm text-gray-500">
                  {filteredProducts.length} {isMobile ? '' : 'results'}
                </span>
              </div>
                    ...
                    >
                      ...
                      <div className="p-2">
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500">
                          ...
                          <span>{product.sold || Math.floor(Math.random() * 1000)} sold</span>
                        </div>
