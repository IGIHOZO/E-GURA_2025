...
const ProductDetail = () => {
  ...
  return (
    <>
      <Helmet>
        {/* Use product's SEO fields if available, otherwise generate from product data */}
        <title>{product.seoTitle || product.metaTitle || `${product.name} - E-Gura Store | Premium Fashion in Kigali, Rwanda`}</title>
        <meta name="description" content={product.seoDescription || product.metaDescription || `${product.name} - ${product.shortDescription || product.description}. Buy ${product.name} online in Kigali, Rwanda. Free shipping, best prices, and premium quality fashion from E-Gura Store.`} />
        <meta name="keywords" content={(product.seoKeywords || product.keywords || []).join(', ') || `${product.name}, ${product.category}, fashion, clothing, Kigali, Rwanda, E-Gura Store, online shopping`} />
        
        {/* Open Graph Meta Tags - Use product's meta tags if available */}
        <meta property="og:title" content={product.metaTags?.ogTitle || product.seoTitle || `${product.name} - E-Gura Store`} />
        <meta property="og:description" content={product.metaTags?.ogDescription || product.seoDescription || `${product.name} - ${product.shortDescription || product.description}. Buy online in Kigali, Rwanda with free shipping.`} />
        <meta property="og:type" content="product" />
        <meta property="og:url" content={window.location.href} />
        <meta property="og:image" content={product.metaTags?.ogImage || product.mainImage || '/logo.png'} />
        <meta property="og:image:width" content="1200" />
        <meta property="og:image:height" content="630" />
        <meta property="og:site_name" content="E-Gura Store" />
        <meta property="og:locale" content="en_US" />
        
        {/* Twitter Card Meta Tags - Use product's meta tags if available */}
        <meta name="twitter:card" content={product.metaTags?.twitterCard || "summary_large_image"} />
        <meta name="twitter:title" content={product.metaTags?.ogTitle || product.seoTitle || `${product.name} - E-Gura Store`} />
        <meta name="twitter:description" content={product.metaTags?.ogDescription || product.seoDescription || `${product.name} - ${product.shortDescription || product.description}. Buy online in Kigali, Rwanda.`} />
        <meta name="twitter:image" content={product.metaTags?.ogImage || product.mainImage || '/logo.png'} />
        ...
        <script type="application/ld+json">
          {JSON.stringify({
            ...
            "@type": "LocalBusiness",
            "name": "E-Gura Store",
            "description": "Premium fashion retailer in Kigali, Rwanda offering the latest trends in clothing and accessories.",
            "url": "https://sewithdebb.com",
