...
const { processProductImages } = require('../utils/imageStorage');
const { v4: uuidv4 } = require('uuid');

// Public SEO Generator Route (no auth required - utility function)
router.post('/generate-seo-public', async (req, res) => {
  try {
...
// Admin authentication required for all routes below - only users with admin role can access
...
router.post('/products/simple', async (req, res) => {
  try {
    ...
    // Convert any base64 images to disk files before DB save
    const productId = uuidv4();
    await processProductImages(productData, productId);
