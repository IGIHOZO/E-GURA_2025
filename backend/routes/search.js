...
const searchProducts = async (req, res) => {
  try {
    ...
    let { query, filters = {}, sortBy = 'relevance', page = 1, limit = 20 } = 
      req.method === 'GET' ? req.query : req.body;

    // Parse filters if it's a JSON string (from GET requests)
...
router.get('/products', cacheResponse(90, {
  keyBuilder: (req) => {
    ...
    return `search:${(req.query.query || '').toLowerCase()}:${JSON.stringify(normalizedFilters)}:${req.query.sortBy || 'relevance'}:${req.query.page || 1}:${req.query.limit || 20}`;
  },
