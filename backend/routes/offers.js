...
const enforceDiscountLimit = (originalPrice, offeredPrice, aiResponse) => {
  const price = Number(originalPrice);
  if (!Number.isFinite(price) || price <= 0 || !aiResponse) {
    return aiResponse;
  }

  const minAllowedPrice = Math.round(price * (1 - MAX_DISCOUNT_PERCENT / 100));
  const updated = { ...aiResponse };

  const clampCounterOffer = (value) => {
    if (!Number.isFinite(value)) return minAllowedPrice;
    return Math.max(Math.round(value), minAllowedPrice);
  };

  if (updated.counterOffer) {
    updated.counterOffer = clampCounterOffer(Number(updated.counterOffer));
  }

  if (updated.decision === 'accept' && Number(offeredPrice) < minAllowedPrice) {
    updated.decision = 'counter';
    updated.counterOffer = clampCounterOffer(updated.counterOffer || minAllowedPrice);
    updated.reasoning = `${updated.reasoning || ''} Adjusted to respect 10% maximum discount.`.trim();
  }

  if (!updated.counterOffer) {
    updated.counterOffer = minAllowedPrice;
  }

  const savings = Math.max(price - updated.counterOffer, 0);
  updated.savings = savings;
  updated.discount = Number(((savings / price) * 100).toFixed(1));

  if (!updated.message || !updated.message.includes('10%')) {
    const limitMessage = `Our best price is ${updated.counterOffer.toLocaleString()} RWF — that's ${MAX_DISCOUNT_PERCENT}% off.`;
    updated.message = updated.message ? `${updated.message}\n\n${limitMessage}` : limitMessage;
  }
...
router.post('/make', async (req, res) => {
  try {
    ...
    // Enforce discount ceiling regardless of negotiation engine
    console.log('🔒 BEFORE enforcement:', { decision: aiResponse.decision, counterOffer: aiResponse.counterOffer, discount: aiResponse.discount });
    aiResponse = enforceDiscountLimit(product.price, offeredPrice, aiResponse);
    console.log('🔒 AFTER enforcement:', { decision: aiResponse.decision, counterOffer: aiResponse.counterOffer, discount: aiResponse.discount });
    console.log('🔒 Original price:', product.price, 'Min allowed:', Math.round(product.price * 0.9));
...
router.post('/chat', async (req, res) => {
  try {
    ...
    // Use intelligent AI to handle conversation with fallback
    let aiResponse;
    try {
      aiResponse = await intelligentAI.handleConversation(chatUserId, message, context);
      ...
      // Fallback response - never stop, always respond
      aiResponse = {
        text: "I'm here to help! Could you rephrase that or tell me what you're looking for? I can help you find products, answer questions, or negotiate prices! 😊",
