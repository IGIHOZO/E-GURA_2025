...
class IntelligentAI {
  ...
  async handleProductSearch(userId, message, entities) {
    ...
    // Extract the actual product name from message - improved pattern
    const productMatch = message.match(/(?:want|need|looking for|find|show me|search for|get me|buy)\s+(?:a\s+|an\s+|some\s+)?(.+?)(?:\s*$)/i);
    if (productMatch) {
      searchQuery = productMatch[1].trim();
    } else if (entities.categories.length > 0) {
      // Use detected category
      searchQuery = entities.categories[0];
    }
    ...
    console.log('🔍 Product search response:', { searchQuery, action: response.action, shouldSearch: response.shouldSearch });
  ...
  async handleGeneral(userId, message, entities) {
    ...
    // If entities found (categories, price), treat as search
    if (entities.categories.length > 0 || entities.priceRange) {
      console.log('✅ Entities detected, triggering product search:', entities);
      return await this.handleProductSearch(userId, message, entities);
    }
    
    // Check if message contains product-related keywords even if not classified
    const productKeywordsFallback = ['want', 'need', 'looking', 'find', 'show', 'search', 'buy', 'get'];
    const hasProductKeyword = productKeywordsFallback.some(keyword => messageLower.includes(keyword));
    
    if (hasProductKeyword) {
      console.log('✅ Product keyword detected, triggering search:', message);
      return await this.handleProductSearch(userId, message, entities);
    }
    ...
    try {
      knowledgeAnswer = await aiKnowledgeBase.answerQuestion(message, { limit: 3 });
    } catch (kbError) {
      console.error('❌ Knowledge base error:', kbError);
      // Continue with default responses instead of failing
    }

    if (knowledgeAnswer && knowledgeAnswer.answer) {
      // Always use knowledge base answer if available
      responses.push(knowledgeAnswer.answer);
    }
    
    // If no knowledge answer or learning needed, record it
    if (knowledgeAnswer && knowledgeAnswer.needsLearning) {
      try {
        await aiKnowledgeBase.recordLearningOpportunity(message, { intent });
      } catch (recordError) {
        console.error('❌ Error recording learning opportunity:', recordError);
      }
    }
